...
int main() {
    ...
    char buff[800010];
    bool rev = false;
    pair<ll, ll> cur = {400000, 400001};
    rep(i, s.size()) buff[cur.first + i] = s[i];
    cur.second = cur.first + s.size();
	rep(_, q) {
		ll t; cin >> t;
		if(t == 1) rev = !rev;
		else{
			ll f; char c; cin >> f >> c;
			if(f == 1) {
				if(!rev) buff[cur.first - 1] = c, cur.first--;
				else buff[cur.second] = c, cur.second++;
			}else{
				if(!rev) buff[cur.second] = c, cur.second++;
				else buff[cur.first - 1] = c, cur.first--;
			}
		}
	}
    if(!rev) {
        for(ll i = cur.first; i <= cur.second - 1; i++) printf("%c", buff[i]);
        printf("\n");
    }
    else {
        for(ll i = cur.second - 1; i >= cur.first; i--) printf("%c", buff[i]);
        printf("\n");
    }
}