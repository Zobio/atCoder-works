...
int main() {
    ...
    vll p = a; // 2回投げたうち1回だけ当たった特典セット
    p.push_back(0); // 2個投げて両方外れた特典
    rep(i, n) rep(j, i, n) {
        if(a[i] + a[j] <= s) p.push_back(a[i] + a[j]);
    }
    sort(all(p));
    ll ans = 0;
    for(auto au : p) {
        ll r = s - au;
        auto it = upper_bound(all(p), r);
        it--; // upper_boundの結果がbegin()でないことは、aの要素の最小値、最大値がそれぞれ0,sであることから保証されている
        chmax(ans, au + *it);
    }
    cout << ans << endl;
}