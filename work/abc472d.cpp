...
// 各安全なマスからのBFSでいけそう
// 0歩で安全が確保されているマスをまず列挙してqueueに入れる
// それらからK歩以内で到達できるマスをBFSで求めていく

vll dy = {-1, 0, 1}, dx = {-1, 0, 1};

int main() {
    ll h, w, k; cin >> h >> w >> k;
    vector<string> s(h);
    rep(i, h) cin >> s[i];
    vll ok_h(h, 1), ok_w(w, 1);
    rep(i, h) rep(j, w) {
        if(s[i][j] == '#') ok_h[i] = ok_w[j] = 0;
    }
    priority_queue<pair<ll, pll>, vector<pair<ll, pll>>, greater<pair<ll, pll>>> pq;
    vvll dist(h, vll(w, LINF));
    rep(i, h) rep(j, w) {
        if(s[i][j] == '.' && ok_h[i] && ok_w[j]) {
            pq.push({0, {i, j}});
            dist[i][j] = 0;
        }
    }
    while(pq.size()) {
        auto [cy, cx] = pq.top().second;
        ll d = pq.top().first;
        pq.pop();
        if(d == k) continue;
        for(auto ddy : dy) for(auto ddx : dx) {
            if(ddy == 0 && ddx == 0) continue;
            if(abs(ddy) + abs(ddx) > 1) continue;
            ll ny = cy + ddy, nx = cx + ddx;
            if(ny < 0 || ny >= h || nx < 0 || nx >= w) continue;
            if(s[ny][nx] == '#') continue;
            if(chmin(dist[ny][nx], d + 1)) pq.push({d + 1, {ny, nx}});
        }
    }
    ll ans = 0;
    rep(i, h) rep(j, w) ans += dist[i][j] <= k;
    //cout << dist << endl;
    cout << ans << endl;
}