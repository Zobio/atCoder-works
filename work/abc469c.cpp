...
int main() {
    ll n; cin >> n;
    string s; cin >> s;
    vll rui_maru(n + 1), rui_batu(n + 1);
    rep(i, n) {
        rui_maru[i + 1] = rui_maru[i] + (s[i] == 'o');
        rui_batu[i + 1] = rui_batu[i] + (s[i] == 'x');
    }
    rep(i, n) {
        ll ans = lower_bound(all(rui_batu), rui_maru[i + 1] + rui_batu[i + 1]) - rui_batu.begin();
        if(ans == n + 1) ans--;
        cout << ans << endl;
    }
}