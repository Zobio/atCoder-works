...
int main() {
    ll n, k; cin >> n >> k;
    priority_queue<pll, vpll, greater<pll>> pq; // 終わる時刻、人数
    dll a(n), b(n), c(n);
    rep(i, n) cin >> a[i] >> b[i] >> c[i];
    ll cur = 0;
    while(a.size() && cur + c.front() <= k) {
        cout << a.front() << endl;
        pq.push({a.front() + b.front(), c.front()});
        cur += c.front();
        a.pop_front(); b.pop_front(); c.pop_front();
    }
    while(pq.size()) {
        pll cust = pq.top();
        cur -= cust.second;
        pq.pop();
        while(a.size() && cur + c.front() <= k) {
            cout << max(cust.first, a.front()) << endl;
            pq.push({max(cust.first, a.front()) + b.front(), c.front()});
            cur += c.front();
            a.pop_front(); b.pop_front(); c.pop_front();
        }
    }
