...
ll dfs(ll p, ll q) {
	...
	dp[p][q] = 0;
	if(p) chmax(dp[p][q], ruia[p] + ruib[q] - dfs(p - 1, q));
	if(q) chmax(dp[p][q], ruia[p] + ruib[q] - dfs(p, q - 1));
	return dp[p][q];
}

int main() {
	cin >> n >> m;
	a.resize(n); b.resize(m); cin >> a >> b;
	reverse(all(a));
	reverse(all(b));
	ruia = mrui(a);
	ruib = mrui(b);
	dp.resize(n + 1, vll(m + 1, -1)); //dp[i][j] : Aの山が残りi枚、Bの山が残りj枚のときの次の出番の人の価値の総和の最大値
	dp.front().front() = 0;
	dfs(n, m);
	//cout << dp << endl;
