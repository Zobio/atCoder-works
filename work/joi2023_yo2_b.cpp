...
int main() {
	...
	ll ans = INF;
	rep(i, 4) rep(j, n) rep(type, 2) { //type==0 --> min
		ll cur = 0;
		rep(k, 4) {
			if(i == k) continue;
			ll p = lower_bound(all(a[k]), a[i][j]) - a[k].begin();
			if(type == 0) {
				if(p == n) cur = INF;
				else chmax(cur, a[k][p] - a[i][j]);
			}
			else{
				if(p > 0) p--;
				if(a[k][p] > a[i][j]) cur = INF;
				else chmax(cur, a[i][j] - a[k][p]);
			}
		}
		chmin(ans, cur);
	}
