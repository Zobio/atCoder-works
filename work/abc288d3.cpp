...
int main() {
	ll n, k; cin >> n >> k;
	vll a(n); cin >> a;
	vll t = {0}; rep(n) t.push_back(a[i]); t.push_back(0);
	vll b(n + 1);
	rep(i, n + 1) b[i] = t[i + 1] - t[i];
	//cout << "b : " << b << endl;
	vvll rui(k, vll(1, 0));
	rep(i, n) rui[i % k].push_back(rui[i % k].back() + b[i]);
	ll q; cin >> q;
	//cout << rui << endl;
	rep(_, q) {
		ll l, r; cin >> l >> r; l--; r--;
		bool flag = true;
		rep(i, k) {
			ll p = (l + i) / k, q = (r - (k - 1 - i)) / k;
			//cout << i << "  " << rui[i][p] << " " << rui[i][q + 1] << endl;
			flag &= rui[i][p] == rui[i][q];
		}
