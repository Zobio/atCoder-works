...
// 最小値を高速で判定するみたいなことをすれば良さそう
// で、そこに必要な移動数を合計してみたいな
// 愚直にやればO(n^ 2)だが、1ずつずれること考えればO(N)に落とせそう
// 左からの最小値、右からの最小値に分けてやるみたいなことをすれば良さそう? 最初の1個目の方が大きい時だけ別途対応
...
int main() {
    ...
    rep(_, t) {
        ...
        ll ans = 0;
        for(ll i = 0; i < n - 1; i++) {
            if(a[i + 1] - a[i] > 1) ans += a[i + 1] - (a[i] + 1), a[i + 1] = a[i] + 1;
        }
        reverse(all(a));
         for(ll i = 0; i < n - 1; i++) {
            if(a[i + 1] - a[i] > 1) ans += a[i + 1] - (a[i] + 1), a[i + 1] = a[i] + 1;
