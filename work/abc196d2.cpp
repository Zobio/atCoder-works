...
int main() {
	LL(h, w, a, b);
	ll ans = 0;
	auto dfs = [&](auto&& self, ll i, ll bits, ll p, ll q) -> void {
		if(i == h * w) {ans++; return;}
		if(bits >> i & 1) return self(self, i + 1, bits, p, q); //既に埋められている
		if(q > 0) self(self, i + 1, bits | 1LL << i, p, q - 1);
		if(p > 0) {
			//横向き
			if(i % w != w - 1 && ~bits & 1 << i + 1) self(self, i + 1, bits | 1LL << i | 1LL << i + 1, p - 1, q);
			//縦向き
			if(i / w != h - 1) self(self, i + 1, bits | 1LL << i | 1LL << i + w, p - 1, q);
		}
	};
	dfs(dfs, 0, 0, a, b);
	cout << ans << endl;
