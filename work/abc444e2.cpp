...
int main() {
    ll n, d; cin >> n >> d;
    vll a(n); cin >> a;
    multiset<ll> ms, m_ms; //後半はlower_boundする用にマイナス値を入れとくためのやつ
    ll ans = 0;
    for(ll l = 0, r = 0; l < n; l++) {
        while(r < n && (ms.lower_bound(a[r]) == ms.end() || *ms.lower_bound(a[r]) - a[r] >= d) && 
                (m_ms.lower_bound(-a[r]) == m_ms.end() || a[r] - (-*m_ms.lower_bound(-a[r])) >= d)) {
            ms.insert(a[r]);
            m_ms.insert(-a[r]);
            r++;
        }
