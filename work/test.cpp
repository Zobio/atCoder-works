...
int32_t main()
{
	...
	vvc s(n, vc(n)); rep(i, n)rep(j, n)cin >> s[i][j];
	...
	while(dq.size())
	{
		const auto[x, y, cst] = dq.front();
		dq.pop_front();
		if(x == gx && y == gy) { cout << cst << endl; return 0; }
		{
			int nx = x, ny = y;
			while(not (++nx < 0 || nx >= n || ++ny < 0 || ny >= n || s[nx][ny] == '#'))if(not vis[nx][ny])dq.eb(nx, ny, cst + 1), vis[nx][ny] = true;
		}
		{
			int nx = x, ny = y;
			while(not (--nx < 0 || nx >= n || ++ny < 0 || ny >= n || s[nx][ny] == '#'))if(not vis[nx][ny])dq.eb(nx, ny, cst + 1), vis[nx][ny] = true;
