...
// Aliceが勝つには
// (1) 数列の要素が全て1 (l=1, r=|a|を選べばそのまま勝ち)
// Bobが勝つには
// (1) 数列の全ての要素が1ではない(l=1, r=|a|を選べばそのまま勝ち)
// だからアリス目線に立つと、「このターンで全ての要素を1に揃えることができなくなった瞬間詰みなので、それを阻止する行動が最適」と言える。(というかそうするしかない)
// だから、毎回Aliceの番は
// 「数列の要素がすべて1でなければ、そのターンで全て1にすることができるか考える。できなかったらその時点でBobのかち」と言える。
// 「全て1にそのターンでできない時点でAliceの負けが確定」はかなり強い条件
// まず、最初のターンでAliceが全ての要素を1(なおかつ要素数が2以上)にできるかどうかの必要十分条件は、
// 「a_1 == 1またはa_n == 1」
...
int main() {
    ...
    rep(_, q) {
        ll n; cin >> n;
        vll a(n); cin >> a;
        if(accumulate(all(a), 0LL) == n) cout << "Alice" << endl;
