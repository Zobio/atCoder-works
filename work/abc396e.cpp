...
//桁ごとに考える
//条件から、各桁について、A_xiとA_yiの(各桁の)数字が同じかどうか(0 0 もしくは 1 1)わかる
//で、矛盾が無ければok(例えばA_1とA_2が逆、A_2とA_3が逆、A_3とA_1が逆、は矛盾)
//↑これを判定するために、適当にA_0とかに0を当てはめて埋めていって、その過程でおかしいことが分かればアウト(-1)

int main() {
	ll n, m; cin >> n >> m;
	vector<pair<pll, ll>> x(m);
	rep(m) {
		cin >> x[i].first.first >> x[i].first.second >> x[i].second;
		x[i].first.first--; x[i].first.second--;
		if(x[i].first.first > x[i].first.second) swap(x[i].first.first, x[i].first.second);
	}
	sort(all(x));
	vll a(n);
	vll al(n);
	al[0] = 1;
	rep(m) {
		if(al[x[i].first.second] || a[x[i].first.second] == a[x[i].first.first] ^ x[i].second) {
			a[x[i].first.second] = a[x[i].first.first] ^ x[i].second;
			al[x[i].first.second] = 1;
		}
		else MINUS;
	}
	cout << a << endl;
