...
// 1と9、2と8..など、xは10-xとペアで考えるだけで良い(1を考えるときに2,3,4とかは気にしないでいいってこと)
// だから1と9のペア、2と8のペア...について隣り合う必要がないように操作する必要がある
// 例えば、1と9しか要素としてなければ絶対に操作が必要
// あと5だけ別扱いになる
...
int main() {
    ...
    rep(_, t) {
        ...
        bool done = false;
        rep(i, 4) {
            if(a[i] && a[8 - i] && sum - a[i] - a[8 - i] == 0) {
                //足して和が10になるペア(1,9),(2,8),(3,7),(4,6)しかないときは間に何か適当に1個挟むのが最適
                cout << 1 << endl;
                done = true;
                break;
            }
        }
        if(!done) cout << max(2 * a[4] - sum - 1, 0) << endl;
    }
}