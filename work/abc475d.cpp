...
// エラトステネスのふるいで10^7未満の素数列挙して判定するだけ?

int main() {
    string s; cin >> s;
    vll isprime(10000010, 1);
    isprime[1] = false;
	for (int p = 2; p <= 10000000; p++) {
        if (!isprime[p]) continue;
        for (int q = p * 2; q <= 10000000; q += p) {
            isprime[q] = false;
        }
    }
    vll a;
    for(ll i = 2; i <= 9999999; i++) if(isprime[i]) a.push_back(i);
    for(auto num : a) {
        string t = to_string(num);
        if(s.size() != t.size()) continue;
        map<char, char> mp; // 各アルファベットが何の数字に対応しているか
        map<char, char> used; // 各数字が何のアルファベットに対応しているか
        bool flag = true;
        rep(i, s.size()) {
            if(mp.find(s[i]) != mp.end()) {
                if(mp[s[i]] != t[i]) flag = false;
            }
            else { //アルファベットが登録されてない
                if(used.find(t[i] - '0') != used.end()) {
                    if(used[t[i] - '0'] != s[i]) flag = false;
                }
                else {
                    mp[s[i]] = t[i];
                    used[t[i] - '0'] = s[i];
