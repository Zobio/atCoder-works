...
// 流石に、除外パターン数を求めて引く方がはやそう！
// 全パターン数は、2^n (nは文字列長)
// グラフ書きたいなあ
// aだったら、a --> b か、 a --> c のイメージ。
// 辺を張って、経路数をDPみたいな?

// 総和から、aがかぶるパターンを引いて、

// dp[i][j] ... i文字目まで見て、最後の文字がjである場合の数で、ansはsum(dp[n]) ??
...
int main() {
    ...
    auto p = encode(s);
    ll n = p.size();
    vector<vector<mint>> dp(n + 1, vector<mint>(3)); //dp[i][j] ... iグループ目まで見て、最後の文字がjである場合の数
    dp[0] = {0, 0, 0}; //空文字列は1通り
    rep(i, n) {
        if(p[i].first - 'a' == 0) {
            dp[i + 1][0] = dp[i][0] + p[i].second + (dp[i][1] + dp[i][2]) * p[i].second;
            dp[i + 1][1] = dp[i][1];
            dp[i + 1][2] = dp[i][2];
        }
        else if(p[i].first - 'b' == 0) {
            dp[i + 1][0] = dp[i][0];
            dp[i + 1][1] = dp[i][1] + p[i].second + (dp[i][0] + dp[i][2]) * p[i].second;
            dp[i + 1][2] = dp[i][2];
        }
        else {
            dp[i + 1][0] = dp[i][0];
            dp[i + 1][1] = dp[i][1];
            dp[i + 1][2] = dp[i][2] + p[i].second + (dp[i][0] + dp[i][1]) * p[i].second;;
        }
    }
    mint ans = 0;
    //rep(i, n + 1) cout << dp[i][0].val() << " " << dp[i][1].val() << " " << dp[i][2].val() << endl;
    rep(i, 3) ans += dp.back().at(i);
    cout << ans.val() << endl;
}