...
int main() {
	...
	ll q; cin >> q;
	vll u(q), v(q); rep(q) cin >> u[i] >> v[i], u[i]--, v[i]--;

	vvll dp(n, vll(n, -LLONG_MAX / 4));
	ll m = 900000000010LL;
	//dp[i][j][k] : i-->jにいくときのコストの最大値
	rep(n) dp[i][i] = 0;
	rep(i, n) rep(j, n) {
		if(s[i][j] == 'Y') {
			dp[i][j] = a[i] + a[j] - m;
	...
	rep(k, n) rep(i, n) rep(j, n) {
		chmax(dp[i][j], dp[i][k] + dp[k][j] - a[k]);
	}

	rep(q) {
		if(dp[u[i]][v[i]] == -LLONG_MAX / 4) cout << "Impossible" << endl;
		else{
			ll d = (-dp[u[i]][v[i]] + m - 1) / m;
			cout << d << " " << dp[u[i]][v[i]] + d * m << endl;
		}
	}
}