...
int main() {
    ...
    rep(_, t) {
        ll n; cin >> n;
        map<pair<ll, char>, ll> ans;
        auto dfs = [&](auto&& self, char pre, ll cnt) -> void {
            if(ans.count({n, pre}) == 0) ans[{n, pre}] = cnt;
            else if(!chmin(ans[{n, pre}], cnt)) return; //もっと早い経路がある
            if(n == 1) return;
            if(pre != 'I') {
                n -= 1;
                self(self, 'I', cnt + 1);
            ...
            if(pre != 'F') {
                if(n % 3 == 0) {
                    n /= 3;
                    self(self, 'T', cnt + 2); // TF
                    n *= 3;
                }
                if(n % 2 == 1) {
                    n = (n - 1) / 2;
                    self(self, 'I', cnt + 2); // IF
                    n = 2 * n + 1;
                }
            }
        };
        dfs(dfs, 'Z', 0); //最初は操作なしで始める
        ll cur = 1, pre = 0; char c = 'I';
        if(ans.count({1, 'T'})) c = 'T';
        if(ans.count({1, 'F'})) c = 'F';
        string s;
        do {
            s.push_back(c);
            ll pp = cur;
            ...
            cout << pre << " " << cur << "  " << c <<endl;
            if(c == 'I') {
                if(ans[{cur, c}] - 1 == ans[{cur * 2, 'T'}]) {c = 'T'; continue;}
                if(ans[{cur, c}] - 1 == ans[{pre + cur, 'F'}]) {c = 'F'; continue;}
            }
            else if(c == 'T') {
                if(ans[{cur, c}] - 1 == ans[{cur + 1, 'I'}]) {c = 'I'; continue;}
                if(ans[{cur, c}] - 1 == ans[{pre + cur, 'F'}]) {c = 'F'; continue;}
            }
            else if(c == 'F') {
                if(ans[{cur, c}] - 1 == ans[{cur + 1, 'I'}]) {c = 'I'; continue;}
