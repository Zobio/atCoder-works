...
int main() {
    ...
    map<pll, ll> pcnt;
    rep(i, m) pcnt[{a[i], b[i]}]++;
    map<ll, vll, greater<ll>> rev;
    rep(i, n) rev[cnt[i]].push_back(i);
    ll ans = 0;
    rrep(i, n) {
        bool fl = false;
        for(auto au : rev) {
            if(fl) break;
            for(auto p : au.second) {
                if(i <= p) {break;}
                if(cnt[i] + cnt[p] < m) {fl = true; break;}
                ans += cnt[i] + cnt[p] - pcnt[{p, i}] >= m;
            }
        }
    }
    cout << ans << endl;
