...
int main() {
	...
	vector<vector<ld>> dp(n + 1, vector<ld>(n + 1)); //dp[i][j] : i回目にj回当たっている確率
	dp.front().front() = 1;
	rep(i, n) rep(j, n + 1) {
		dp[i + 1][j] = dp[i][j] * (1 - p[i]) + (j ? dp[i][j - 1] * p[i] : 0);
	}
	cout << accumulate(dp.back().begin() + n / 2 + 1, dp.back().end(), (ld)0) << endl;
}