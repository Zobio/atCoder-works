...
int main() {
    ...
    vector<modint> kaijou(max(k, n) + 1);
    kaijou[0] = kaijou[1] = 1;
    for(ll i = 2; i <= max(k, n); i++) kaijou[i] = kaijou[i - 1] * i;

    vector<modint> rui(n + 1);
    rep(i, n) rui[i + 1] = rui[i] + a[i];

    modint ans = 0;
    rep(i, n) {
        ans += kaijou[n - 1] / (kaijou[n - k] * kaijou[k - 1]) * a[i] * a[i];
    }
    rep(i, n - 1) {
        ans += 2 * kaijou[n - 2] / (kaijou[n - k] * kaijou[k - 2]) * a[i] * (rui.back() - rui[i + 1]);
    }
    cout << ans.val() << endl;
}