...
int main() {
	//最大値が変わるタイミングでは、山の高さが確定する
	//最大値が変わらないタイミングでは、山の高さは最大値以下の任意の値
	//なので、各iについて、山の高さの上界を求めていって、かけ合わせればいい 矛盾が1個でもあれば0
	ll n; cin >> n;
	vll a(n), b(n); cin >> a >> b;
	reverse(all(a));
	a.push_back(-LINF);
	...
	rrep(i, n) {
		if(b[i] != b[i + 1]) {
			if(r[i] != -1 && r[i] != b[i]) {cout << 0 << endl; return 0;}
			else if(r[i] == -1 && i < n - 1 && h[i] < b[i]) {cout << 0 << endl; return 0;}
			else r[i] = b[i], chmin(h[i], b[i]);
		}
		else chmin(h[i], b[i]);
	}
	mint ans = 1;
