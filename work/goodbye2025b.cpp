...
// N=2*10^5なので、O(NlogN)くらい?
// 最小の回数u-->sにかえる
// 端っこのuはどう頑張ったところでuに変えるしかない
// 結局、やらなければいけないことは
// "連続するu"を排除すること と、 "端っこのu"を削除すること
// だから、最初に末端処理して、あとは貪欲にやってけばOK...? (直感的には貪欲でいけそう)
...
int main() {
    ...
    rep(_, q) {
        ...
        ll n = s.size();
        ll ans = 0;
        if(s.front() == 'u') ans++, s.front() = 's';
        if(s.back() == 'u') ans++, s.back() = 's';
        rep(i, 1, n - 1) if(s[i - 1] == 'u' && s[i] == 'u') ans++, s[i] = 's';
        cout << ans << endl;
    }
}