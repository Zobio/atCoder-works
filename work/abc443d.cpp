...
// 結局やることは、
// 全てのiについて |A_i - A_(i + 1)| <= 1
// ってこと

// これ、実は|差-1|を合計取ってくだけ?
//　次のやつが上に行くか下に行くかまで見る必要がありそう?

// 1. まず、frontを次の要素に合わせて変えて、それ以降は右が左に合わせる
// 2. 最初から右が左に合わせる
// 上記をreverseしたAについてもやって4通り出した中での最小値が答え?
...
int main() {
    ...
    rep(_, t) {
        ...
        // まずは、1.のパターン
        if (abs(a[0] - a[1]) > 1) {
            if(a[0] > a[1]) sum += a[0] - (a[1] + 1), a[0] = a[1] + 1;
            else sum += (a[1] - 1) - a[0], a[0] = a[1] - 1;
        }
        for(ll i = 2; i < n; i++) {
            if(abs(a[i - 1] - a[i]) <= 1) continue;

            if(a[i] > a[i - 1]) sum += a[i] - (a[i - 1] + 1), a[i] = a[i - 1] + 1;
            else sum += (a[i - 1] - 1) - a[i], a[i] = a[i - 1] - 1;
        }
        chmin(ans, sum);
