...
template<class T> void arrcout(T &a) { for(size_t i = 0; i < a.size(); i++) cout << (i ? " " : "") << a.at(i).val(); cout << endl; }
...
int main() { //配るDP
	ll n, pp; cin >> n >> pp;
	mint p = pp;
	vector<mint> dp(n + 1); //dp[i] : 体力をi減らすのにかかる期待値
	rep(i, n) {
		dp[i + 1] += dp[i]* (1 - p / 100) + 1; //normal
		dp[min(i + 2, n)] += dp[i] * (p / 100) + 1; //critical
	}
	arrcout(dp);
