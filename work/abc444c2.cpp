...
// Lを決めた時に、「おらないでも良いのは1つだけ」。なぜなら、折らなくて良いのは元々Lだった時に限るから。
// つまり、A_n(とA_nと同じ数のA_i)以外は必ず二つ接合してLを構築しなければいけない
// かつ、和のパターンが端っこ同士で接合していくしかない(なぜならそうじゃないと接合した長さが単調増加になるところがある)から、

bool all_same(vll a) {
    bool flag = true;
    rep(a.size() - 1) flag &= a[i] == a[i + 1];
    return flag;
}

bool check(vll a, ll l) {
    if(a.size() % 2 == 1) return false;
    bool flag = true;
    rep(a.size() / 2) {
        flag &= a[i] + a[a.size() - 1 - i] == l;
    }
    return flag;
}
...
int main() {
    ...
    if(all_same(b)) {
        ...
        ll l = b.back();
        while(b.back() == b[b.size() - 2]) b.pop_back();
        b.pop_back();
        if(check(b, l)) cout << l << " ";
    }
    if(check(a, a.front() + a.back())) cout << a.front() + a.back();
    cout << endl;
}