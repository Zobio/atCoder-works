...
int main() {
	...
	while(n > 1) {
		bool fl = false;
		for(ll i = 2; i * i <= n; i++) {if(n % i == 0) {a.push_back(i); n /= i; fl = true; break;}}
		if(!fl) break;
	}
	...
	for(auto cur : mp) {
		...
		ll lim = cur.second;
		for(ll i = num;;i+=num) {
			if(rest.count(i) == 0) rest[i] = i;
			while(rest[i] % num == 0 && cnt < lim) cnt++, rest[i] /= num;
			if(cnt >= lim) {chmax(ans, i); break;}
		}
	}
	cout << ans << endl;
} 