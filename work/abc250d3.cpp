...
template<class T>
vector<T> enum_prinmes(T num) {
	//num以下の素数を列挙する
	vector<bool> m(num + 1); //合成数かどうか
	for(T i = 2; i * i <= num; i++) {
		for(T j = i * 2; j <= num; j += i) m[j] = true;
	}
	vector<T> ret;
	for(T i = 2; i <= num; i++) if(!m[i]) ret.push_back(i);
	return ret;
}
...
int main() {
	...
	__int128_t ma = 1;
	while(ma * ma * ma <= n) ma++;
	vector<__int128_t> p = enum_prinmes(ma);
	ll ans = 0;
	rep(i, p.size()) {
		ll l = i, r = p.size();
		while(r - l > 1) {
			ll mid = l + r >> 1;
			if(p[i] * p[mid] * p[mid] * p[mid] <= n) l = mid;
			else r = mid;
		}
		ans += l - i;
	}
