...
// 気持ちとして、正の数はfirst、負のすうはsecondでなるべく処理したい
// 今secondに負の数が来ていて、それをfirstに持っていくメリットはあるか?
// --> 別にぱっと見なさそう
// secondが負の数だったらさっさと処理、じゃあsecondが生の数だったら?

// 最初の要素は絶対に+に、最後の要素は絶対に-になるって点で、例外的な扱いかも
// 逆に言えば、左端っ子は+、端以外部分はabs、右端っこは-で、その中で大きいn-1個を取るだけ?

// ただ、「端以外部分はabs」になるためには、左の要素を+として処理していることが必要
// だから、
// (1)左端を+として処理(これは確実に選択)、端以外部分はabs、右端っこは-の中から不必要な1個以外選択
// (2)左端を選ばない、端以外は全て-で選択(不必要なし)
// で、max((1), (2))する?
