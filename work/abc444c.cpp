...
// Lの候補は実は少なく、
// (i) Nが基数の時、a.back()のみ
// (ii) Nが偶数の時、a.back()と、a.front()+a.back()の2つ
...
int main() {
    ...
    sort(all(a));
    if(all_same(a)) {
        if(n % 2 == 1) cout << a.back() << endl;
        else cout << a.back() << " " << a.back() * 2 << endl;
        return 0;
    }
    while(a.back() == a[a.size() - 1 - 1]) a.pop_back();
    n = a.size();
    if(n % 2 == 1) {
        cout << a.back() << endl;
        return 0;
    }
    if(all_same(a)) cout << a.back() << " ";
    bool flag = true;
    rep(i, a.size() / 2) {
        flag &= a[i] + a[n - 1 - i] == a.front() + a.back();
    }
    if(flag) cout << a.front() + a.back();
    cout << endl;
}