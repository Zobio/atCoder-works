...
//右に石がなくて左にだけあってもしょうがないので、右から詰めていく貪欲でOK?
//後ろから見ていく O(N)が無理だからイベントソートみたいなことを考える
//石がある間マス同士の長さ > 石の個数 ならアウト　そうじゃなければコスト1+2+...+(マス同士の長さ-1)
//↑一番遠いところに石1個を異動させる→次に遠いところに石1個を移動させる... を繰り返すと考えると妥当


int main() {
	ll n, m; cin >> n >> m;
	vpll a(m);
	rep(m) cin >> a[i].first, a[i].first--;
	rep(m) cin >> a[i].second;
	a.push_back({n, 0});

	if (a.front().first != 0) MINUS; //先頭に石を配置できない

	ll t = 0;
	rep(m) t += a[i].second;
	if(t != n) MINUS; //1個ずつ置けない

	ll ans = 0;
	rrep(m) {
		ll l = a[i + 1].first - a[i].first;
		if(l < a[i].second) MINUS; //その区間の石が過剰なので、左が足りなくなるはず
		ll lack = l - a[i].second;
		ans += l * (l - 1) / 2 - lack * (lack - 1) / 2;
		a[i].first += lack; //足りない分区間変更
	}
