...
int main() {
	//ナップサック問題
	//dp[i]　: i議席を獲得するのに必要な最小の鞍替えの人数
	ll n; cin >> n;
	vll x(n), y(n), z(n);
	rep(n) cin >> x[i] >> y[i] >> z[i];
	map<ll, ll> dp;
	dp[0] = 0;
	rep(i, n) {
		ll cur = x[i] > y[i] ? 0 : (y[i] - x[i] + 1) / 2;
		vpll tmp;
		for(auto au = dp.rbegin(); au != dp.rend(); au++) {
			ll gi = au -> first;
			ll cost = au -> second;
			tmp.push_back({gi + z[i], dp[gi] + cur});
		}
		for(auto t : tmp) {
			if(dp.count(t.first)) chmin(dp[t.first], t.second);
			else dp[t.first] = t.second;
		}
	}
	ll sum = accumulate(all(z), 0ll);
	ll ans = LINF;
	for(auto au : dp) {
		if(au.first * 2 < sum) continue;
		chmin(ans, au.second);
	}
	cout << ans << endl;
}