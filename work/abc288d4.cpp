...
int main() {
	...
	vvll rui(k, vll(1, 0));
	rep(i, b.size()) rui[i % k].push_back(rui[i % k].back() + b[i]);
	ll q; cin >> q;
	cout << a << endl << b << endl;
	cout << rui << endl;
	rep(_, q) {
		...
		rep(i, k) {
			ll cur = (l + i) % k;
			ll p = (l + i) / k, q = r / k + (r % k >= cur) - 1;
			cout << cur << "  " << p << " " << q << "  "
				 << rui[cur][p] + (i == 0 ? a[l] : 0) << " " << rui[cur][q + 1] - (i == k - 1 ? a[r + 2] : 0) << endl;
			fl &= rui[cur][q + 1] - rui[cur][p] + (i == 0 ? a[l] : 0) + (i == k - 1	 ? a[r + 2] : 0) == 0;
		}
		cout << (fl ? "Yes" : "No") << endl;
	}
}