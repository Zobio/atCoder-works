...
// 最初に各マスについて、塗りかえOK/NGを求めておいて、
// 後ろからOKなやつを塗っていって、NG-->OKも追っていけばよさそう
...
int main() {
    ...
    rep(_, q) {
        ...
        if(t == 1) {
            ...
            char x; cin >> x;
            query.push_back({t, x - 'a'});
        }
    }
    vll ok(n, 1); //ぬれるか
    rep(i, q) {
        if(query[i].first == 1) ok[query[i].second] = 1 - ok[query[i].second];
    }
    vll done(n, 0);
    rep(i, n) if(ok[i]) done[i] = 1;
    set<ll> que; //次色塗られたら変更するまちリスト indexを入れる
    rep(i, n) if(done[i]) que.insert(i);
    rep(i, n) done[i] = 0;
    rrep(i, q) {
        if(query[i].first == 2) {
            while(que.size()) {
                ans[*que.begin()] = query[i].second;
                done[*que.begin()] = 1;
                que.erase(que.begin());
            }
        }
        else {
            ok[query[i].second] = 1 - ok[query[i].second];
            if(!done[query[i].second]) {
                if(ok[query[i].second]) {
                    que.insert(query[i].second);
                }
                else {
                    que.erase(query[i].second);
                }
            }
        }
    }
    string s;
    rep(i, n) s.push_back('a' + ans[i]);
    cout << s << endl;
}