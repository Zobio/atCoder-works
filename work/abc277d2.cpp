...
int main() {
	...
	vll sum = {a.front()};
	rep(i, n - 1) {
		if(a[i + 1] - a[i] <= 1) sum.back() += a[i + 1];
		else sum.push_back(a[i + 1]);
	}
	ll ans = *max_element(all(sum));
	if(a.front() == 0 && a.back() == m - 1 && sum.size() >= 2) chmax(ans, sum.front() + sum.back());
	cout << accumulate(all(a), 0ll) - ans << endl;
}