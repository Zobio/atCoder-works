...
vll dy = {-1, 0, 1};
vll dx = {-1, 0, 1}; // (0, 0)になるところはスキップして使う

int main() {
    ll h, w; cin >> h >> w;
    ll n = h * w;
    vector<string> s(h); cin >> s;
    ll cnt = 0; // 黒になる時刻がもとまったものの個数
    vvll t(h, vll(w, -1));
    ...
    rep(i, h) rep(j, w) {
        if(s[i][j] == '#') {
            ...
            for(auto ddy : dy) for(auto ddx : dx) {
                auto [ny, nx] = make_pair(i + ddy, j + ddx);
    ...
    while(que.size() && cnt < n) {
        auto [cy, cx] = que.front().first;
        ll time = que.front().second;
        que.pop();
        for(auto ddy : dy) for(auto ddx : dx) {
            if(ddy == 0 && ddx == 0) continue;
            auto [ny, nx] = make_pair(cy + ddy, cx + ddx);
            if(ny < 0 || ny >= h || nx < 0 || nx >= w) continue;
            if(s[ny][nx] == '.') {
                s[ny][nx] = '#';
                if(t[ny][nx] == -1) t[ny][nx] = time + 1, cnt++;
