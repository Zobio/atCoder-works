...
template<typename T>
vector<pair<T, ll>> RLE(vector<T> arr) {
	/*
	ランレングス圧縮をする。
...
int main() {
	ll n; cin >> n;
	string s; cin >> s;
	vector<char>t; rep(i, n) t.push_back(s[i]);
	auto r = RLE(t);
	ll ans = 0;
	//rep(i, r.size()) cout << (char)r[i].first << " " << r[i].second << endl;
	for(auto au : r) ans += au.second * (au.second - 1) / 2;
	cout << ans << endl;
}