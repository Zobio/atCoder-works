...
// 今見てる文字の先頭がeかr以外だったら無理
// 今見てる文字の先頭がeの場合、eraserにできるなら絶対する(先頭がrになったら詰むから) できないならerase
// 今見てる文字の先頭がdの場合、dreamしか無理ならそうする。dreamerにできる場合、erの後の文字がaの時だけdream、dかeならdreamer
...
int main() {
    ...
    ll cur = 0;
    while(true) {
        if(s[cur] != 'd' && s[cur] != 'e') NO;
        if(n - cur < 5) NO;

        if(s.substr(cur, 5) == "erase") {
            if(n - cur < 6 || s.substr(cur, 6) != "eraser") cur += 5;
            else cur += 6;
        }
        else if(s.substr(cur, 5) == "dream") {
            if(n - cur < 7 || s.substr(cur, 7) != "dreamer") cur += 5;
            else if(n - cur >= 8 && s[cur + 7] == 'a') cur += 5;
            else cur += 7;
        }
        else NO;
