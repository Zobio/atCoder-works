...
int main() {
	...
	vll p(d), c(d); rep(d) cin >> p[i] >> c[i], c[i] /= 100;
	vvll dp(d + 1, vll(400010, LINF));
	dp[0][0] = 0;
	rep(i, d) rep(point, 200001) {
		rep(j, p[i] + 1) {
			chmin(dp[i + 1][point + (i + 1) * j + (j == p[i]) * c[i]], dp[i][point] + j);
		}
	}
	ll ans = LINF;
	rep(i, g, 200010) chmin(ans, dp[d][i]);
	cout << ans << endl;
}