...
// 流石に自明に、黒も白も大きい方からなるべくたくさん持っておくのが良さそうなので。
// 白の個数でループ回して、
// 黒の個数は、白の個数だけ取ったときにすでにマイナスを取らなければいけないならそこまでで、プラスが残っているなそれもとる

int main() {
    ll n, m; cin >> n >> m;
    vll b(n), w(m); cin >> b; cin >> w;
    sort(rall(b)); sort(rall(w));
    auto rui_b = mrui(b);
    auto rui_w = mrui(w);
    ll bmax = *max_element(all(rui_b));
    ll ans = max(0LL, bmax);
    rep(i, m) {
        if(i + 1 > n) break; // そんなに黒玉取れないから終わり
        if(b[i] < 0) { //余計に取らない(取らなければいけないマイナスまでは取る)
            chmax(ans, rui_w[i + 1] + rui_b[i + 1]);
        }
        else {
            chmax(ans, rui_w[i + 1] + bmax); //なるべくとる
        }
    }
    cout << ans << endl;
}