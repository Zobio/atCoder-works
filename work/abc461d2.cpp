...
int main() {
    ...
    ll ru = 0, rv = 0;
    ll ans = 0;
    for(ll i1 = 0; i1 < h; i1++) for(ll i2 = i1 + 1; i2 < h + 1; i2++) {
        ru = 0, rv = 0;
        for(ll l = 0; l < w; l++) {
            chmax(ru, l + 1);
            chmax(rv, l + 1);
            while(ru < w + 1 && rui[i2][ru] - rui[i2][l] - rui[i1][ru] + rui[i1][l] < k) ru++;
            while(rv < w + 1 && rui[i2][rv] - rui[i2][l] - rui[i1][rv] + rui[i1][l] <= k) rv++;
            //if(rv == w + 1 && rui[i2].back() - rui[i2][l] - rui[i1].back() + rui[i1][l] <= k) v++;
            //cout << i1 << " " << i2 << "  " << l << " " << ru << " " << rv << endl;
            ans += rv - ru;
        }
    }
    cout << ans << endl;
