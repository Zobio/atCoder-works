...
void dfs(ll bits, ll p) {
    if(p + 1 < h * w) {
        if((bits >> (p + 1) & 1) == 0) {
            ll nxt_bits = bits | (1LL << p) | (1LL << (p + 1));
            place_ok.insert(nxt_bits);
            ll pp = p + 2;
            while(pp < h * w) {
                if((nxt_bits >> pp & 1) == 0) dfs(nxt_bits, pp);
                pp++;
            }
        }
    }
    if(p + w < h * w) {
        if((bits >> (p + w) & 1) == 0) {
            ll nxt_bits = bits | (1LL << p) | (1LL << (p + w));
            place_ok.insert(nxt_bits);
            ll pp = p + 1;
            while(pp < h * w) {
                if((nxt_bits >> pp & 1) == 0) dfs(nxt_bits, pp);
...
int main() {
    cin >> h >> w;
    a.resize(h, vll(w)); cin >> a;
    place_ok.insert(0); //置いて良いパターンをbitで持っておく (前計算) 右端のbitが(0,0)に対応していることに注意
    dfs(0, 0); //今の状態が000..00で、1番左(左上)にある未確定の場所が0
    cout << place_ok << endl;
    // bit全探索で、「それがおいて良いパターンか?」を判定した後で、OKなら実際に累積XORを計算して、MAXとって終わり
}