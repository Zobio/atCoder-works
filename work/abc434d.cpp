...
// b[i][j] ... (0. 0)から(i, j)までで、上にある雲のの個数が1である座標の個数 を計算
// 答えは、bを利用することでO(1)で求めることができる。
...
int main() {
    ...
    ll cnt_0 = 0;
    rep(i, 2000) rep(j, 2000) cnt_0 += a[i][j] == 0;
    rep(i, n) {
        cout << cnt_0 + imos2[d[i] + 1][r[i] + 1] - imos2[d[i] + 1][l[i]] - imos2[u[i]][r[i] + 1] + imos2[u[i]][l[i]] << endl;
    }
}