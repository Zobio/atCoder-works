...
int main() {
	...
	vll ruiA(n + 1), ruiB(m + 1);
	rep(n) ruiA[i + 1] = ruiA[i] + a[i];
	rep(m) ruiB[i + 1] = ruiB[i] + b[i];

	ll p = m; //マイナスに変わるポインタ
	rep(m) if(b[i] < 0) {p = i; break;}

	vll ma(m + 1); // ma_i:白がi個目まで使えるときの総和の最大値
	for(ll i = 0; i <= m; i++) {
		if(i < p) ma[i] = ruiB[i];
		else ma[i] = ruiB[p];
	}
	...
	ll ans = 0;
	for(ll i = 0; i <= n; i++) chmax(ans, ruiA[i] + ma[min(i, m)]);
	cout << ans << endl;
}