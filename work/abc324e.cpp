...
//左側だったらtの何文字目まで行けるか、右側だったらtの何文字目から行けるか、を考える
...
int main() {
	...
	vll en, st; //何文字目までいけるか、何文字目からいけるか
	rep(i, n) {
		ll c = 0;
		rep(j, s[i].size()) {
			if(c == l) break;
	...
	ll ans = 0;
	rep(i, n) {
		ans += n - (lower_bound(all(st), l - en[i]) - st.begin());
	}
	cout << count(all(en), l) << endl;
	ans += count(all(en), l) * (n - count(all(en), l));
	cout << ans << endl;
}