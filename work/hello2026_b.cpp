...
// 0があるならば、絶対0個にしてはいけない。 (確実に最適行動でなくなる)
// その上で1があるならば、絶対0個にしてはいけない。 (同様に)
// もしかすると、結局最初の0,1,2...って見ていってその時のmexがそのまま適用されるのかもしれない? (全体のmexってこと)
// 要素削除の順番が最終的な結果に影響しないし、範囲は指定されるがそこの中のどの要素を選んでも良いのがでかい
// min(mex(a), k - 1)が答え?
// 部分mexについて、それがkならk-1を削除するだけ(どうせ残らないので)
// 部分mexについて、それがkでないならば、重複かoverしてる値があるからそれを削除するだけ(どうせ答えに寄与しないから)
// だから、結局min(mex(a), k - 1)が答えで良さそう
...
int main() {
    ...
    rep(_, q) {
        ll n, k; cin >> n >> k;
        vll a(n); cin >> a;
        map<ll, ll> mp; rep(n) mp[a[i]]++;
        ll mex = 0;
        rep(i, n + 1) if(mp[i] == 0) {mex = i; break;}
